...
show_help() {
  ...
  printf "  %-28s %s\n" "kitupdate [--full]" "Sync kit files (--full: regenerate templates)"
  ...
  echo ""
  echo -e "  ${DIM}init → claude → prd → design → techstack → todo → build → test → review → audit → predeploy → deploy${NC}"
  echo ""
  echo -e "  ${CYAN}1. Setup${NC}      kit init → kit claude/cursor/windsurf"
  echo -e "  ${CYAN}2. Plan${NC}       /prd → /design → /techstack → /todo"
  echo -e "  ${CYAN}3. Build${NC}      /build → /recipe → /test"
  echo -e "  ${CYAN}4. Review${NC}     /review → kit audit → kit sweep"
  echo -e "  ${CYAN}5. Ship${NC}       /preflight → kit predeploy → /deploy"
  echo -e "  ${CYAN}6. Maintain${NC}   /debug → /learn → /changelog → kit update --full"
  echo ""
