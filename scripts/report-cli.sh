...
if [ "$DO_CLEAR" = true ]; then
  ...
  DELETED=0
  CUTOFF=$(date -u +%Y-%m-%d -d "30 days ago" 2>/dev/null || date -u -v-30d +%Y-%m-%d 2>/dev/null || echo "")
  if [ -z "$CUTOFF" ]; then
...
if [ "$DO_GITHUB" = true ]; then
  ...
  # Build markdown body
  BODY="## SaasCode Kit Issue Report\n\n"
  BODY="${BODY}**Date range:** Last ${DAYS} day(s) | **Total:** ${ENTRY_COUNT} issues\n\n"

  # Critical issues section
  if [ "$CRIT_COUNT" -gt 0 ]; then
    BODY="${BODY}### Critical Issues\n\n"
    echo "$ALL_ENTRIES" | jq -r 'select(.severity == "critical") | "- [ ] **[\(.source)]** \(.category): \(.message)" + (if .file != "" then " (`\(.file)`)" else "" end)' 2>/dev/null | while IFS= read -r LINE; do
      BODY="${BODY}${LINE}\n"
      echo "$LINE"
    done | {
      LINES=""
    ...
    # Re-read critical entries for body
    CRIT_LINES=$(echo "$ALL_ENTRIES" | jq -r 'select(.severity == "critical") | "- [ ] **[\(.source)]** \(.category): \(.message)" + (if .file != "" then " (`\(.file)`)" else "" end)' 2>/dev/null)
    BODY="${BODY}### Critical Issues\n\n${CRIT_LINES}\n\n"
  fi

  # Warning issues section
  if [ "$WARN_COUNT" -gt 0 ]; then
    WARN_LINES=$(echo "$ALL_ENTRIES" | jq -r 'select(.severity == "warning") | "- [ ] **[\(.source)]** \(.category): \(.message)" + (if .file != "" then " (`\(.file)`)" else "" end)' 2>/dev/null)
    BODY="${BODY}### Warnings\n\n${WARN_LINES}\n\n"
  fi

  BODY="${BODY}---\n*Generated by [saascode-kit](https://github.com/AshGw/saascode-kit) report*"

  # Rebuild body properly using a temp file
  TMPBODY=$(mktemp)
  {
