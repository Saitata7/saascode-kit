...
cmd_docs_prd() {
  ...
  echo ""
  echo "${BOLD}Product Brief Generator${NC}"
  echo ""
  ...
  if [ -f "$PRD_FILE" ]; then
    echo "  ${YELLOW}⚠${NC}  Product brief already exists: $PRD_FILE"
    echo "  ${DIM}Delete it first to regenerate, or edit it directly.${NC}"
    echo ""
    return
  fi
  ...
  if [ -n "$IDEA" ]; then
    cat > "$PRD_FILE" << EOF
<!-- MODE: ${MODE_LABEL} -->
<!-- Generated by: saascode docs --prd -->
<!-- AI Agent: Read the instructions below, then replace this entire file with the generated Product Brief -->

# Product Brief — GENERATE THIS
...
You are generating a **${MODE_LABEL}** Product Brief for the idea above.

Follow the structure below IN ORDER. Each section builds on the previous one.
...
1. **Problem** — 2-4 pain points for specific actors
2. **Solution** — 2-4 answers mapping to problems. Last line: full lifecycle
3. **Entities & Fields** — Every entity with fields, types, enums, foreign keys
4. **Relationships** — How entities connect (1:1, 1:many, many:many)
5. **Domain Rules** — Business constraints grouped by category
6. **State Machine** — ASCII diagram + transition table for every status field
7. **Core Flow** — 3-column diagram (Actor A | Platform | Actor B)
8. **v1 Scope** — Build vs Skip with reasons
9. **Build Order** — Implementation sequence (auth first, admin last)

### Mode: ${MODE_LABEL}
$(if [ "$MODE_LABEL" = "deep" ]; then
cat << 'DEEP'

**Deep mode — research deeply into each entity:**
- For every entity: explain WHY each field exists, cite industry patterns
- For every enum: explain each value, when it transitions, who triggers it
- For domain rules: include compliance references, legal requirements, edge cases
- For state machines: include timeout, retry, manual override scenarios
- For relationships: include matching signals and filtering strategies
- For scope: explain WHY each item is deferred (needs data, complex, dependency)
- If idea involves AI/automation: include AI Agents section with v1/v2 approach
- Research the domain — cite competitors, regulations, industry standards
DEEP
else
cat << 'STANDARD'

**Standard mode — complete PRD, all sections:**
- Cover all entities the system needs
- List essential fields with types and enums
- Define the domain rules that make the system correct
- State machine for every entity with status fields
- Core flow showing happy path
...
- Field names: camelCase (ready for code)
- Enum values: UPPER_SNAKE (ready for code)
- Foreign keys: \`fieldId → Entity\`
- Optional fields: marked with \`?\`
- Important fields: **bold** with explanation
- No TODO markers — make assumptions, note them
- End with: *This is the complete picture. Everything else is detail.*

### Quality Checklist

- [ ] Every solution bullet maps to a problem bullet
- [ ] Every entity appears in relationships
- [ ] Every status enum has a state machine
- [ ] Core flow reaches a terminal state
- [ ] No orphaned entities
- [ ] v1 scope delivers core value end-to-end

---

**NOW: Replace this entire file with the generated Product Brief. Do not keep these instructions.**
EOF

    echo "  ${GREEN}✓${NC} Created: $PRD_FILE"
    echo "  ${CYAN}Mode:${NC} ${MODE_LABEL}"
    echo "  ${CYAN}Idea:${NC} ${IDEA}"
    echo ""
    echo "  ${BOLD}Next steps:${NC}"
    echo "  1. Open your AI tool (Claude Code, Cursor, Windsurf)"
    echo "  2. Tell the AI: ${CYAN}\"Generate the product brief from docs/product/product-brief.md\"${NC}"
    echo "  3. The AI will read the instructions and generate a complete PRD"
    echo ""

    # If Claude Code skill exists, show skill option
    if [ -d "$ROOT/.claude/skills" ] || [ -n "$SAASCODE_KIT_DIR" ]; then
      echo "  ${DIM}Claude Code users: Use /prd to generate directly${NC}"
    fi

  else
    # No idea provided — show usage
    echo "  ${CYAN}Usage:${NC}"
    echo "    saascode docs --prd \"your idea here\""
    echo "    saascode docs --prd --deep \"your idea here\""
    echo ""
    echo "  ${CYAN}Examples:${NC}"
    echo "    saascode docs --prd \"Task management app for small teams\""
    echo "    saascode docs --prd --deep \"AI-powered legal marketplace with escrow payments\""
    echo ""
    echo "  ${CYAN}What it does:${NC}"
    echo "    Creates docs/product/product-brief.md with your idea and"
    echo "    structured instructions for any AI tool to generate a"
    echo "    complete Product Brief (PRD)."
    echo ""
    echo "  ${CYAN}Modes:${NC}"
    echo "    --prd          Standard (100-200 lines, focused)"
    echo "    --prd --deep   Deep (300-500 lines, comprehensive)"
    echo ""
  fi
}
...
cmd_help() {
  ...
  printf "  %-28s %s\n" "/prd [idea]" "Generate Product Brief (PRD)"
  printf "  %-28s %s\n" "/prd --deep [idea]" "Generate comprehensive PRD"
