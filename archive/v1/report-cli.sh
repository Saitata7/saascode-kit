...
if [ "$DO_GITHUB" = true ]; then
  ...
  WARN_COUNT=$(echo "$ALL_ENTRIES" | jq -r '.severity' 2>/dev/null | grep -c '^warning$') || WARN_COUNT=0
  TODAY=$(date -u +%Y-%m-%d)
  TITLE="[kit] Issue Report: ${CRIT_COUNT} critical, ${WARN_COUNT} warning ($TODAY)"

  # Build markdown body via temp file
  TMPBODY=$(mktemp)
  {
    echo "## Kit Issue Report"
    echo ""
    ...
    echo "---"
    echo "*Generated by [kit](https://github.com/Saitata7/saascode-kit) report*"
  } > "$TMPBODY"
